...
const postReserva = (req,res)=>{
    ...
    // antes de crear la reserva ver si esta disponible ese ambiente y ver si el tipo de usuario que autoriza es de tipo 3 si no, no me deja guardar 
    Usuario.findOne({
        where:{
            usu_id: objReserva.usu_autoriza
        }        
    }).then(usuario_autoriza=>{
        console.log(usuario_autoriza);
        ...
        if(usuario_autoriza.usu_tipo===3){
            Reserva.findAll({
                where:{
                    ...
                    [Op.or]:[{
                        res_fechin : {[Op.between]: [objReserva.res_fechin,objReserva.res_fecha_fin]}
                    },{
                        res_fechfin: {[Op.between]: [objReserva.res_fechin,objReserva.res_fecha_fin]}
                    }]}
            }).then(reservas=>{
            ...
            res.status(401).json({
                ok:false,
                mensaje:'el usuario no disponible de privilegios sufucientes para reservar el aula'
            })
        }
    })
}
